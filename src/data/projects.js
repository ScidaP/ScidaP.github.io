export const projects = [
  {
    id: 1,
    title: 'Sistema para bares',
    titleEn: 'Bar Management System',
    description: 'Sistema integral de gestión para bares y restaurantes. Administra pedidos, mesas, mozos y transacciones con comunicación en tiempo real entre cocina, personal y clientes. Muy pronto al lanzamiento.',
    descriptionEn: 'Comprehensive management system for bars and restaurants. Manages orders, tables, waitstaff and transactions with real-time communication between the kitchen, staff and customers. Launching very soon.',
    tags: ['React', 'ASP.NET Core', 'PostgreSQL', 'SignalR', 'Entity Framework', 'JWT'],
    category: 'web',
    github: 'https://github.com/ScidaP/BarMaster',
    privateRepo: true,
    demo: null,
    devTime: '+2 años',
    devTimeEn: '+2 years',
    images: [
      '/img/sistema_bares/2dcdb433176efc9a3ecdcf02c82638de.png',
      '/img/sistema_bares/4d83b7f38e6b57c2846f7e4e203d6fbf.png',
      '/img/sistema_bares/d30d418f75bb5b6814fc6d205126e10c.png',
      '/img/sistema_bares/f05c64bd16390013a7615d43e1b5d780.png',
    ],
  },
  {
    id: 2,
    title: 'GPU remote access',
    titleEn: 'GPU remote access',
    description: 'Aplicación de escritorio para gestionar conexiones remotas a equipos con GPU. Permite agregar, editar y eliminar conexiones IP con autenticación de usuarios. Creado inicialmente para aprender sobre desarrollo con Python. La idea nace de una propuesta real de un cliente.',
    descriptionEn: 'Desktop application to manage remote connections to GPU-equipped computers. Allows adding, editing and deleting IP connections with user authentication. Initially created to learn Python desktop development. The idea came from a real client proposal.',
    tags: ['Python', 'PySide6', 'PostgreSQL', 'Qt'],
    category: 'desktop',
    github: 'https://github.com/ScidaP/gpu-remote-access',
    demo: null,
    devTime: '1 mes',
    devTimeEn: '1 month',
    images: [
      '/img/gpu-remote-access/0be4eb955abd62aa62e5cd35533ba4e9.png',
      '/img/gpu-remote-access/453293f0f156329ff377ebb262a261f0.png',
      '/img/gpu-remote-access/b46d9bdbf513f1c7f320b59cf644ea01.png',
    ],
  },
  {
    id: 3,
    title: 'Sistema de manejo de construcciones',
    titleEn: 'Construction Management System',
    description: 'Sistema de gestión de obras desarrollado para una empresa española. Permite administrar obras, empresas, profesionales y usuarios con reportes y carga de archivos.',
    descriptionEn: 'Construction management system developed for a Spanish company. Allows managing projects, companies, professionals and users with reports and file uploads.',
    tags: ['PHP', 'MySQL', 'Bootstrap', 'jQuery', 'DataTables'],
    category: 'web',
    github: 'https://github.com/ScidaP/db-user-interface',
    demo: null,
    devTime: '1 año y 2 meses',
    devTimeEn: '1 year and 2 months',
    images: [
      '/img/sistema_obras/c676d378885a75765e55e48902103789.png',
      '/img/sistema_obras/d0572959932bdac7e324447c547ad8b1.png',
      '/img/sistema_obras/e2b5fad2cb7d8cf1f783b45d5e0c74f4.png',
    ],
  },
  {
    id: 4,
    title: 'Este portfolio',
    titleEn: 'This portfolio',
    description: 'Portfolio personal construido con Claude Code. El objetivo fue simplemente ambientarme en el desarrollo moderno de aplicaiones, con asistencia absoluta de IA pero con intensa supervisión humana.',
    descriptionEn: 'Personal portfolio built with Claude Code. The goal was simply to get familiar with modern application development, with full AI assistance but intense human supervision.',
    tags: ['React', 'Vite', 'Tailwind CSS'],
    category: 'web',
    github: 'https://github.com/ScidaP/ScidaP.github.io',
    demo: 'https://scidap.github.io',
    devTime: 'En progreso',
    devTimeEn: 'In progress',
    images: [
      '/img/portfolio/40a75dade6438784404054f1443560e9.png',
    ],
  },
  {
    id: 5,
    title: 'LoL Timer',
    titleEn: 'LoL Timer',
    description: '',
    descriptionEn: '',
    tags: [],
    category: 'web',
    github: '#',
    demo: null,
    images: [],
  },
]
