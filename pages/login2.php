...
<head>
	...
	<!-- RECAPTCHA -->
    <script src="https://www.google.com/recaptcha/enterprise.js?render=6LeUEGgpAAAAAPH_rWpxG_pbwjmlQuruVGAqyF16"></script>
	<!-- RECAPTCHA END -->
	<script>
  function onClick(e) {
    e.preventDefault();
    grecaptcha.enterprise.ready(async () => {
      const token = await grecaptcha.enterprise.execute('6LeUEGgpAAAAAPH_rWpxG_pbwjmlQuruVGAqyF16', {action: 'LOGIN'});
    });
  }
</script>
</head>
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if (isset($_POST['dni'])) {
			function post_captcha($user_response) {
				$fields_string = '';
				$fields = array(
					'secret' => '6LfjuGMpAAAAAGdhDqVAXX5UgiN4hossLY65CuL-',
			...
			if (!$res['success']) {
				...
				$errormail = "<span style='color:red;'>Incorrect validation.</span> Refresh the page <a class='text-span-12' href='$signup'>here</a>.";
					...
					<div class="row justify-content-center">
						<div class="col-md-6 col-lg-5 mb-5 mb-lg-0">
							...
							<form action="<?=$login?>" id="frmSignIn" method="post" class="needs-validation">
								...
								<div class="row">
									<div class="form-group col">
										<button  class="inline-flex items-center justify-center relative transition uppercase font-bold tracking-wider rounded-[5px] whitespace-nowrap focus:outline-none h-[55px] bg-[#3546DF] text-white hover:bg-[#4A5AE2] active:bg-[#313EAF] mt-4 w-full g-recaptcha" data-v-6ea5bc34="" data-sitekey="6Lfun1YhAAAAANwBnwrmJLlDQ6__ZPAREc0CKiJW" data-v-6ea5bc34="" style="height:100px;width:100px;" data-callback="onSubmit">

										<button type="submit" class="btn btn-dark btn-modern w-100 text-uppercase rounded-0 font-weight-bold text-3 py-3" data-loading-text="Cargando...">Iniciar</button>
									</div>
								</div>
							</form>
						</div>
